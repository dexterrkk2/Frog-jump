...
{
    public Nullable<Vector3> CalculateFiringSolution(Vector3 start, Vector3 end, float muzzleVelocity, Vector3 gravity)
    {
        Nullable<float> ttt = GetTimeToTarget(start, end, muzzleVelocity, gravity);
        if (!ttt.HasValue)
        {
            return null;
        }
